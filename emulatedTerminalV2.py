  ...
  def update_game(self):
    ...
    if self.fight_game.is_fight_time(self.player_run)[0] :
      if key == Qt.Key.Key_Space :
        ...
        Enemy.shoot_enemy(self.fight_game.is_fight_time(self.player_run)[1], self.buffer, self.game_run.get_fight())
    ...
    # Use dt from game_run.get_diff_time() directly for timer updates
    self.timer_game.remove_time(dt) 
    ...
    if self.fight_game.is_fight_time(self.player_run)[0] :
      Enemy.draw_Enemy(self.buffer, self.game_run.get_fight(), self.player_run, Color(0,255,159)) # Pass blue_cyber
