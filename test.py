import math, sys, os, time, curses
...
def endGame(window, game, death, stdscr):
...
def draw_sentence(window_inp,game_inp,player_inp,npc,sentence,color, stdscr):
...
def ask_question(window_inp, game_inp, npc, sentence, color, stdscr):
  padding = 12
  x_shift = 60
...
def talk_to_NPC(window_inp,player_inp,game_inp,npc,color,stdscr):
  for sentence in NPC.get_texts(npc) :
    if sentence[0] == 'FUNC':
      ...
      draw_sentence(window_inp,game_inp,player_inp,npc,sentence,color,stdscr)
  
  if NPC.get_type(npc) == 3 :
    endGame(window_inp,game_inp,1, stdscr)

  Game.get_npcs(game_inp).remove(npc)   

def draw_NPC(window_inp, game_inp, player_inp, talk_color, stdscr):
  for npc_g in game_inp.get_npcs() :
    ...
    if 0.01 < distance < 2.5 :
      if distance > 1 :
        ...
        talk_to_NPC(window_inp, player_inp, game_inp, npc_g, talk_color, stdscr)
...
def run(stdscr):
  ...
  if mystery_npc:
    talk_to_NPC(buffer_window, player_run, game_run, mystery_npc, blue_cyber, stdscr)

  starting_game_time = time.time()
  
  # Boucle de simulation :
  while True :
    interact(game_run,player_run,buffer_window,stdscr)
    ...
    if Game.get_map(game_run)[int(Player.get_position(player_run)[1])][int(Player.get_position(player_run)[0])] :
      refresh_buffer(buffer_window)
      endGame(buffer_window,game_run, 0, stdscr)
      break
    ...
    draw_NPC(buffer_window,game_run,player_run,blue_cyber,stdscr)
